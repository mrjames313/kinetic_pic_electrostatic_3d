...
//    }).sum();
    ...
    pub fn load_particles_box(&mut self, corner_min: DVec3, corner_max: DVec3,
        ...
        let vel_rewind_factor = 0.5 * world.time().dt() * self.charge / self.mass;
        ...
        for _i in 0..num_sim_particles {
            ...
            let full_idx : DVec3 = world.world_spec().get_full_node_index(pos);
...
//        println!("Stats of box counts: min {}, max {}", min, max);
    ...
    pub fn advance(&mut self, world : &ThreeDWorld) {
        let dt = world.time().dt();
        let charge_per_mass = self.charge / self.mass;
        
        for particle in self.particles.iter_mut() {
            let mut full_idx : DVec3 = world.world_spec().get_full_node_index(particle.pos);
            let ef = world.interpolate_ef(full_idx);
            ...
            full_idx = world.world_spec().get_full_node_index_no_assert(particle.pos);

            if full_idx[0] < 0.0 {
                particle.pos[0] = 2.0 * world.world_spec().x_dim().min() - particle.pos[0];
                particle.vel[0] *= -1.0;
            } else if full_idx[0] >= (world.world_spec().x_dim().n()-1) as f64 {
                particle.pos[0] = 2.0 * world.world_spec().x_dim().max() - particle.pos[0];
                particle.vel[0] *= -1.0;
            }
            if full_idx[1] < 0.0 {
                particle.pos[1] = 2.0 * world.world_spec().y_dim().min() - particle.pos[1];
                particle.vel[1] *= -1.0;
            } else if full_idx[1] >= (world.world_spec().y_dim().n()-1) as f64 {
                particle.pos[1] = 2.0 * world.world_spec().y_dim().max() - particle.pos[1];
                particle.vel[1] *= -1.0;
            }
            if full_idx[2] < 0.0 {
                particle.pos[2] = 2.0 * world.world_spec().z_dim().min() - particle.pos[2];
                particle.vel[2] *= -1.0;
            } else if full_idx[2] >= (world.world_spec().z_dim().n()-1) as f64 {
                particle.pos[2] = 2.0 * world.world_spec().z_dim().max() - particle.pos[2];
                particle.vel[2] *= -1.0;
            }
        }
    }
